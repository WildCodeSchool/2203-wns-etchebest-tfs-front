...
test.describe('Struture App', () => {
	test('Should register then logout then login', async ({ page, browserName }) => {
		...
		const register = page.getByRole('button', { name: "S'inscrire" })
		await register.click({ delay: 1000 })
		delay(2000).then(async () => {
			...
			await page.getByPlaceholder('Saisissez votre mot de passe').click()
			await page.getByPlaceholder('Saisissez votre mot de passe').fill('123456789')
			const login = page.getByRole('button', { name: 'Connexion' })
			await login.click()
		})
	})

	test('Should disabled button when register form is not valid', async ({
		page,
		browserName
	}) => {
		await page.goto('http://localhost:3000/register')
		...
		expect(await page.getByRole('button', { name: "S'inscrire" })).toHaveAttribute(
			'disabled',
			''
		)
