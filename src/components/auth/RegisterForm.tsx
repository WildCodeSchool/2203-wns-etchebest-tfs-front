...
export function RegisterForm() {
	...
	//Form
	const { register, handleSubmit, watch, formState: { errors } } = useForm<IRegisterFormData>({ mode: 'onTouched' })
	const validators = {
		firstname: {
			required: {
				value: true,
	...
	//Mutation
	const [mutateRegister, { data, loading, error: ApolloError }] = useMutation(REGISTER_USER)
	...
	return (
		<form className='grid grid-cols-2 gap-3 ' onSubmit={onSubmit}>
			...
			<div className="col-span-1">
				...
				/>
				<ErrorInput errors={errors} field='firstname' />
			</div>
			...
			<div className="col-span-1">
				...
				/>
				<ErrorInput errors={errors} field='lastname' />
			</div>
			...
			<div className='col-span-2 mb-5'>
				<label htmlFor='confirm-password' className={styles.label}>Confirmez le mot de passe</label>
				<input
				  id='confirm-password'
				  ...
				  autoComplete="new-password"
				/>
				{isEmpty(!errors) && <ErrorInput errors={errors} field='confirmPassword' />}
			</div>
			<div className='col-span-2'>
				<Button type='submit' fullWidth disabled={!isEmpty(errors)} loading={loading}>S'inscrire</Button>
				{ApolloError && <p className='px-4 py-2 text-white text-sm font-medium rounded-sm bg-alert'>{ApolloError.message}</p>}
			</div>
		</form>
	)
}
